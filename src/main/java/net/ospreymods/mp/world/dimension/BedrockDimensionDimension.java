...
package net.ospreymods.mp.world.dimension;

import org.jline.terminal.Size;

import net.ospreymods.mp.item.BedrockDimensionItem;
...
public class BedrockDimensionDimension extends MpModElements.ModElement {
	@ObjectHolder("mp:bedrock_dimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("mp:bedrock_dimension_portal")
	...
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("mp:bedrock_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("mp:bedrock_dimension"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("mp:bedrock_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("mp:bedrock_biome")),};
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = BedrockDimensionDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("bedrock_dimension_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(Blocks.BEDROCK.getDefaultState().getBlock(),
